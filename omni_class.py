        ...
        for sample in range(10):  # num of episodes
            ...
            for i in range(self.batchsz):  # one batch means one set
                ...
                for j, cur_class in enumerate(selected_cls):
                    ...
                    selected_img = np.random.choice(20, self.k_shot, False)

                    # meta-training and meta-test
                    x_spt.append(data_pack[cur_class][selected_img[:self.k_shot]])
                    
                    query = data_pack[cur_class][selected_img[:self.k_shot]]
                    query = np.squeeze(np.squeeze(query, axis=0),axis=0)
...
if __name__ == '__main__':
    ...
    db = OmniglotNShot('/home/atik/Documents/MAML/Summer_1/datasets/Omniglot/', 
                       batchsz=20, n_way=5, k_shot=1, k_query=1, imgsz=28)
    ...
    for i in range(1000):
        ...
        plt.figure(1)
        plt.imshow(x_spt[0][1].transpose(2, 0).numpy())
        plt.pause(0.5)
        plt.figure(2)
        plt.imshow(x_qry[0][1].transpose(2, 0).numpy())
        plt.pause(0.5)
