...
if __name__ == '__main__':
    ...
    # print(net)
    # load model
    net.load_state_dict(torch.load('ckpt/compare51_unsup_new.mdl'))
    ...
    partial_path = '/home/atik/Documents/UMAML_FSL/data/unsup_100/'
    
    mini = MiniImagenet(partial_path, 
                      mode='train', n_way=n_way, k_shot=k_shot, k_query=k_query,
                batchsz=1000, resize=224)
    
    mini_val = MiniImagenet(partial_path, 
                      mode='test', n_way=n_way, k_shot=k_shot, k_query=k_query,
                    batchsz=20, resize=224)
    ...
    for epoch in range(100):
        ...
        for step, batch in enumerate(db):
            ...
            if step % 200 == 0:
                ...
                print('<<<<>>>>accuracy:', accuracy.cpu().detach().numpy(), 'best accuracy:', 
                      best_accuracy.cpu().detach().numpy())
